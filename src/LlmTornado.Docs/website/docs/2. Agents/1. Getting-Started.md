...
2. **Execution**: Agent processes user input via `Run()`
3. **Tool Resolution**: Agent determines if tools are needed
4. **Response Generation**: Agent produces `Conversation` based on context
5. **State Management**: Agent is Stateless but `Conversation` history can be added to `Run()`
...
Conversation result = await agent.Run("Explain async/await in C#");
Console.WriteLine(result.Messages.Last().Content);
```
...
// First interaction
Conversation result = await agent.Run("My name is Alice.");
Console.WriteLine(result.Messages.Last().Content);
...
**Solutions**:
- Set `streaming: true` in constructor or `Run`
...
- `Run(string input, ...)` - Execute agent with user input
...
- `DelegateReference` - Delegate tools supplied on construction
- `ToolList` - Built and registered tool definitions
