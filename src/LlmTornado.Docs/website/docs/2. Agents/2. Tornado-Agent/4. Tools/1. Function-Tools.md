...
Conversation result = await agent.Run("What's the weather in Prague?");
```
...
Tool checkBank = new Tool(
    ([SchemaAnyOf(typeof(PayPal), typeof(BankTransfer))] IList<IPaymentMethod> paymentMethod, ToolArguments args) =>
    {
        ...
        return $"Payment processed using {paymentMethod.First().GetType().Name}";
    }
);
...
agent.AddTool(checkBank);
agent.AddTool(getOrder);
...
permissions["dangerous_operation"] = true;  // Requires permission
permissions["safe_operation"] = false;      // No permission needed
