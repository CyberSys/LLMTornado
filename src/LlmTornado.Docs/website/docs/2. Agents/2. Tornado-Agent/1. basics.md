...
// Run the agent
Conversation result = await agent.Run("Hello, what can you help me with?");
...
Conversation result = await agent.Run("What's the weather like in Prague?");
...
Conversation result = await agent.Run("Analyze: Build a REST API with authentication");
...
// First interaction
Conversation result = await agent.Run("My name is Alice and I'm learning C#.");
...
// Use initial schema
Conversation result = await agent.Run("Extract: John, 30, NYC");

// Change schema dynamically
agent.UpdateOutputSchema(typeof(CompanyInfo));

// Use new schema
result = await agent.Run("Extract: TechCorp, founded 2010, 500 employees");
```
...
Conversation result = await agent.Run("What's the weather in London?");
```
...
try
{
    Conversation result = await agent.Run(userInput);
...
- `Run(string input, ...)` - Execute agent with input
...
#### `List<Delegate>` DelegateReference
