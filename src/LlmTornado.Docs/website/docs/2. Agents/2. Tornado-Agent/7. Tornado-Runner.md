...
// TornadoRunner is used internally when calling Run
Conversation result = await agent.Run("Your query here");
```
...
ValueTask HandleRunnerEvents(AgentRunnerEvents runEvent)
{
    switch (runEvent.EventType)
    {
        ...
        case AgentRunnerEventTypes.ToolInvoked:
            ...
            {
                Console.WriteLine($"Tool called: {toolEvent.ToolCalled.Name}");
            }
            break;
